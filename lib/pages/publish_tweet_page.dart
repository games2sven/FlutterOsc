...
class _PublishTweetPageState extends State<PublishTweetPage> {
  ...
  void _publishTweet(BuildContext context,String token) async{
    ...
    if(fileList.length > 0){
      for(File file in fileList){
        ...
        multipartRequest.files.add(http.MultipartFile('img',stream,length,filename: fileName));
      }
    }
    setState(() {
      isLoading = true;
  ...
  void _pickImage(BuildContext context) {
    ...
    if(size >= 9){
      Scaffold.of(context).showSnackBar(new SnackBar(content: new Text("最多只能添加9张图片！")));
      return;
    }
